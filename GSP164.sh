...
# 4.1 Despues de implementar la API le enviamos solicitudes con el siguiente script.
./query_api.sh

# 4.2 Para correr una prueba con los parametros de consulta iataCode ejecutamos el siguientescript
./query_api.sh JFK
...
# 6.1 Aplicamos la configuracion de quota con sl siguiente script.
...
# Creo mi API KEY utilizando el parametro de la variable
...
# Enviamos una nueva solicitus utilizando la API KEY que creamos
./query_api_with_key.sh $API_KEY

### SEGMENTO 4
# Ahora la API tiene una cutoa limite de 5 solicitudes por segundo. ejecutamos el siguiente comando para enviar trafico a la API.
# Esperamos 5 o 10 segundos para detener el proceso con las teclas Ctrl+C
./generate_traffic_with_key.sh $API_KEY


### SEGMENTO 5
#Enviamos una solictud autenticada ala API.
